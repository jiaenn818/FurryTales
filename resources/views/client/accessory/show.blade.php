...
<link rel="stylesheet" href="{{ asset('css/pet-show.css') }}">
<link rel="stylesheet" href="{{ asset('css/accessory-show.css') }}">

<div class="pet-show-page">
    
    <!-- Back Button -->
    <a href="{{ route('client.accessories.index') }}" class="pet-show-back-button">
        ...
        <div class="pet-show-images">
            <div class="pet-show-header pet-show-header-left">
                ...
                <div class="pet-show-meta">
                    <span class="pet-show-breed">{{ $accessory->Category }}</span>
                    <span class="pet-show-divider"></span>
                    <span class="pet-show-price">RM {{ number_format($accessory->min_price, 2) }}</span>
                </div>
                <p class="text-brand-medium brand-text">Brand : {{ $accessory->Brand }}</p>
            </div>

            <div class="pet-show-main-image">
                <!-- Main Image -->
                <img id="mainPhoto" src="{{ asset($accessory->ImageURL1 ?: 'image/default-pet.png') }}" alt="{{ $accessory->AccessoryName }}">
        ...
        <div class="pet-show-details">
            <div class="pet-show-details-card">
                ...
                @if(isset($details) && count($details) > 0)
                    <div class="pet-show-selections selection-container">
                        @foreach($details as $label => $values)
                            <div class="selection-group">
                                <h3 class="selection-header">{{ $label }}</h3>
                    ...
                    <script>
                        document.addEventListener('DOMContentLoaded', function() {
                            const variants = @json($accessory->variants);
                            ...
                            const initialMinPrice = {{ $accessory->min_price ?? 0 }};
                            ...
                            function updateUI() {
                                ...
                                if (variant) {
                                    ...
                                    let stockHtml = '';
                                    const accessoryOutlets = @json($accessory->outlets);
                ...
                <div class="pet-show-description-container">
                    ...
                    <p class="pet-show-description-text">{{ $accessory->Description }}</p>
                </div>
                ...
                <div class="pet-show-section">
                    ...
                    <form id="addToCartForm" action="{{ route('client.cart.add', $accessory->AccessoryID) }}" method="POST" class="pet-show-action-form action-form">
                        @csrf
...
{{-- Success/Error Modal (Mirroring Pet Details) --}}
    ...
    <div class="pet-show-modal-wrapper">
        ...
        <div class="pet-show-modal-content">
            ...
            <div id="cartModalOptions" class="pet-show-modal-options">
                <a href="{{ route('client.cart.view') }}" class="pet-show-modal-button pet-show-modal-button-primary">Go to Cart</a>
                <a href="{{ route('client.accessories.index') }}" class="pet-show-modal-button pet-show-modal-button-secondary">Continue Browsing</a>
            </div>
...
<script>
    let imgs = [
        @if($accessory->ImageURL1) "{{ asset($accessory->ImageURL1) }}", @endif
        @if($accessory->ImageURL2) "{{ asset($accessory->ImageURL2) }}", @endif
        @if($accessory->ImageURL3) "{{ asset($accessory->ImageURL3) }}", @endif
        @if($accessory->ImageURL4) "{{ asset($accessory->ImageURL4) }}", @endif
        @if($accessory->ImageURL5) "{{ asset($accessory->ImageURL5) }}", @endif
    ].filter(Boolean);
