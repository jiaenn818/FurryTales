    ...
    <div class="container">
        ...
        <div class="cart-items-section">
            <div class="cart-header">
                <h2>My Cart</h2>
                <span class="cart-item-count">{{ $cartItems->count() }} Items</span>
            </div>

            @if(session('success'))
                <div class="cart-alert cart-alert-success">
                    <p>{{ session('success') }}</p>
                    @if(session('showOptions'))
                        <div class="cart-alert-links">
                            <a href="{{ route('client.cart.index') }}">View Cart</a>
                            <a href="{{ url()->previous() }}">Continue Browsing</a>
                        </div>
                    @endif
                </div>
            @endif
              ...
              @foreach($cartItems as $item)
                ...
                <div class="cart-item">
                    ...
                    <div class="cart-col info">
                        <h3>{{ $name }}</h3>
                        <p>{{ $isPet ? $product->Breed : $product->Brand }}</p>


                        @if(!$isPet && $item->variant)
                            <div class="variant-selector">
                                <select class="cart-variant-select" data-cart-item-id="{{ $item->CartItemID }}">
                                    @if($item->availableVariants)
                                        @foreach($item->availableVariants as $availVariant)
                                            @php
                                            ...
                                            @endphp
                                            <option 
                                                value="{{ $availVariant->VariantID }}" 
                                                {{ $isCurrentVariant ? 'selected' : '' }}
                                                {{ $isOutOfStock ? 'disabled' : '' }}
                                                data-stock="{{ $availVariant->StockQty }}"
                                            >
                        ...
                        @if($item->OutletID)
                            <div class="outlet">📍 {{ $item->outlet->City }}</div>
                        @endif
                    </div>
                    ...
                    <div class="cart-col qty">
                        @if($isPet)
                            {{ $quantity }}
                        @else
                            <div class="cart-qty-controls" data-cart-item-id="{{ $item->CartItemID }}" data-max-stock="{{ $stockQty }}">
                                <button class="cart-qty-btn cart-qty-minus" data-action="decrement">&minus;</button>
                                <input type="number" class="cart-qty-input" value="{{ $quantity }}" min="1" max="{{ $stockQty }}" data-unit-price="{{ $unitPrice }}">
                    ...
                    <div class="cart-col remove">
                        <form action="{{ route('client.cart.remove', $item->CartItemID) }}" method="POST">
                            @csrf
                ...
                @if($cartItems->isEmpty())
                    <div class="cart-empty">
                        ...
                        <a href="{{ route('client.pets.index') }}">Browse Pets</a>
                        <a href="{{ route('client.accessories.index') }}">Browse Accessories</a>
                    </div>
                @endif
            </div>
        ...
        <div class="cart-summary">
            ...
            {{-- Logistics Section (Moved to Right) --}}
            <div class="cart-logistics-container cart-logistics-item">
                ...
                <div class="cart-address-section">
                    <div class="cart-form-section">
                        ...
                        <div class="cart-form-grid cart-contact-grid">
                            <input type="text" placeholder="Name" id="contact-name" value="{{ Auth::user()->name }}" class="cart-form-input">
                            <input type="text" placeholder="Phone Number" id="contact-phone" value="{{ Auth::user()->phoneNo }}" class="cart-form-input cart-pickup-fields">
                        </div>
                    </div>
                </div>
                ...
                <div class="cart-address-section">
                    ...
                    <div id="delivery-fields" class="cart-form-section cart-delivery-fields">
                        ...
                        <textarea placeholder="Address (Street, Unit, etc.)" id="delivery-address" rows="3" class="cart-form-textarea cart-delivery-address">{{ Auth::user()->customer->address ?? '' }}</textarea>
                ...
                <form id="checkout-form" action="{{ route('client.payment.checkout') }}" method="POST" class="hidden">
                    @csrf
        ...
        <div id="vouchers-list" class="vouchers-list">
            @forelse($availableVouchers as $v)
                <div class="voucher-card" 
                     data-code="{{ $v->voucherID }}" 
                     data-min-spend="{{ $v->minSpend }}"
                     onclick="selectVoucher('{{ $v->voucherID }}', {{ $v->minSpend }})">
                    <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 0.5rem;">
                        <div>
                            <span class="voucher-code">{{ $v->voucherID }}</span>
                            <span class="voucher-min-spend">Min. Spend RM {{ number_format($v->minSpend, 2) }}</span>
                        </div>
                    ...
                    <div style="display: flex; justify-content: space-between; align-items: flex-end;">
                        <span class="voucher-amount">RM {{ number_format($v->discountAmount, 2) }} OFF</span>
                        <span class="voucher-expiry">Exp: {{ $v->expiryDate ? $v->expiryDate->format('d M Y') : 'No Expiry' }}</span>
                    </div>
                </div>
            @empty
...
window.appliedDiscount = {{ session('applied_voucher')['discountAmount'] ?? 0 }};
window.appliedVoucherID = '{{ session('applied_voucher')['voucherID'] ?? "" }}';
@php
    $minSpend = 0;
...
@endphp
window.appliedMinSpend = {{ $minSpend }};
