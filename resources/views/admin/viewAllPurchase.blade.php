    ...
    <div class="container">
        ...
        <table class="table table-bordered table-striped">
            ...
            <tbody>
                ...
                <tr data-purchase="{{ strtolower($purchase->PurchaseID) }}" data-customer="{{ strtolower($purchase->CustomerID) }}" data-date="{{ \Carbon\Carbon::parse($purchase->OrderDate)->format('Y-m-d') }}">
                    ...
                    <td>
                        ...
                        @if ($purchase->Method === 'Delivery' && $purchase->riderID)
                        <span class="badge-method text-muted" style="margin-left:6px;">
                            Rider: {{ $purchase->riderID }}
                        </span>
                        @endif
                        ...
                        <ul class="mb-0">
                            ...
                            <li>
                                @if($item->pet)
                                <strong>Pet:</strong> {{ $item->pet->PetID }} -- [{{ $item->pet->PetName }}] ({{ $item->Quantity }} x RM {{ number_format($item->Price,2) }})
                                @elseif($item->accessory)
                                <strong>Accessory:</strong> {{ $item->accessory->AccessoryID }} -- [{{ $item->accessory->AccessoryName }}]
                                @if($item->variant)
                                ...
                                @endif
                                @if($item->outlet)
                                <span class="text-muted">[Outlet: {{ $item->outlet->OutletID }}]</span>
                                @endif
                            </li>
                            @endforeach
                        </ul>
                    ...
                    <td>
                        ...
                        <span class="badge-method">
                            <small>{{ $purchase->DeliveredDate }}</small>
                        </span>
                    </td>
                </tr>
                @empty
