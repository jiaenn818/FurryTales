...
@section('title', 'Your Rides Management')
...
<main class="admin-main">
    <div class="container">
        ...
        <div class="rides-container" id="ridesContainer">
            ...
            <div class="ride-card {{ $index === 0 ? 'active' : 'inactive' }}"
                data-purchase-id="{{ $purchase->PurchaseID }}"
                id="card-{{ $purchase->PurchaseID }}">
                <div class="ride-card-header">
                    <h3>
                        Purchase #{{ $purchase->PurchaseID }}
                        <span class="status-badge status-{{ strtolower($purchase->Status) === 'delivered' ? 'delivered' : 'in-progress' }}">
                            {{ $purchase->Status }}
                        </span>
                    </h3>
                ...
                <div class="card-content">
                    <div class="basic-info">
                        <div class="info-row">
                            ...
                            <span class="info-value">{{ $purchase->AddressOnly }}</span>
                        </div>
                        <div class="info-row">
                            <span class="info-label">Receiver:</span>
                            <span class="info-value">{{ $purchase->Receiver }}</span>
                        </div>
                        <div class="info-row">
                            <span class="info-label">Phone:</span>
                            <span class="info-value">{{ $purchase->PhoneNumber }}</span>
                        </div>

                        <div class="info-row">
                            <span class="info-label">Postcode:</span>
                            <span class="info-value">{{ $purchase->Postcode }}</span>
                        </div>
                    </div>
                    ...
                    <div class="detail-info">
                        <div class="info-row">
                            ...
                            <span class="info-value">{{ $purchase->OrderDate ?? 'Not available' }}</span>
                        </div>

                        <button class="btn btn-primary" onclick="showRoute('{{ $purchase->AddressOnly }}')">
                            View Route
                        </button>
                    </div>
                    ...
                    <div class="action-container">
                        ...
                        <form action="{{ route('admin.rider.purchase.delivered', $purchase->PurchaseID) }}" method="POST" class="delivery-form">
                            @csrf
                            @method('PATCH')
        ...
        <div class="text-center mt-4">
            <p class="text-muted">{{ count($purchases) }} ride(s) assigned to you</p>
        </div>
        @endif
    </div>
</main>
...
<script>
    ...
    navigator.geolocation.getCurrentPosition(function(position) {
        ...
        const iframeUrl = `https://www.google.com/maps/embed/v1/directions?key={{ env('GOOGLE_MAPS_API_KEY') }}&origin=${lat},${lng}&destination=${encodeURIComponent(address)}`;
...
document.addEventListener('DOMContentLoaded', function () {
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(function(position) {
            ...
            fetch(`{{ route('admin.rider.weather.coords') }}?lat=${lat}&lng=${lng}`)
                .then(res => res.json())
                .then(data => {
