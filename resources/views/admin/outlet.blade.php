...
@section('content')
    <div class="container-fluid">
        ...
        <div class="stats-grid">
            ...
            <div class="stat-card">
                <h3>{{ number_format($averagePets, 1) }}</h3>
                ...
                <table class="table">
                    ...
                    <tbody>
                        @forelse($outlets as $outlet)
                            <tr>
                                <td>
                                    <strong>{{ $outlet->OutletID }}</strong>
                                </td>
                                <td>
                                    <span class="badge badge-primary">{{ $outlet->State }}</span>
                                </td>
                                <td>
                                    <strong>{{ $outlet->City }}</strong><br />
                                    {{ $outlet->Postcode }}
                                </td>
                                <td>
                                    <span class="badge badge-success">{{ $outlet->pets_count ?? 0 }} pets</span>
                                    <span class="badge badge-info">{{ $outlet->accessories_count ?? 0 }} accessories</span>
                                </td>
                                <td>
                                    <div class="action-buttons">
                                        <button class="btn btn-sm btn-warning"
                                            onclick="openModal('edit', '{{ $outlet->OutletID }}')">
                                            <i class="fas fa-edit"></i> Edit
                                        </button>
                                        <!-- CHANGE THIS LINE: Call confirmDelete with the outlet ID -->
                                        <button class="btn btn-sm btn-danger"
                                            onclick="confirmDelete('{{ $outlet->OutletID }}')">
                                            <i class="fas fa-trash"></i> Delete
                                        </button>
                                        <button class="btn btn-sm btn-info"
                                            onclick="viewOutlet('{{ $outlet->OutletID }}')">
        ...
        <div class="modal-content">
            ...
            <form id="outletForm" method="POST">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="OutletID" class="form-control" value="{{ $nextOutletID }}" disabled>
                    <input type="hidden" name="OutletID" value="{{ $nextOutletID }}">
                </div>
                <div class="form-group">
                    <label for="State"><i class="fas fa-map-marker-alt"></i> State *</label>
                    <select id="State" name="State" class="form-control" required>
                        <option value="">Select State</option>
                        @foreach (['Johor', 'Kedah', 'Kelantan', 'Melaka', 'Negeri Sembilan', 'Pahang', 'Perak', 'Perlis', 'Penang', 'Selangor', 'Terengganu', 'Kuala Lumpur', 'Labuan', 'Putrajaya'] as $state)
    ...
    `).join('');
        ...
        function openModal(type, outletId = '') {
            ...
            if (type === 'add') {
                ...
                form.action = "{{ route('admin.outlets.store') }}"; 
