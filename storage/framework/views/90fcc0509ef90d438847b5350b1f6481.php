            ...
            <tbody>
                <?php $__empty_1 = true; $__currentLoopData = $suppliers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $s): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <tr>
                        ...
                        <td><?php echo e($s->SupplierName); ?></td>
...
<?php $__env->startPush('scripts'); ?>
    <script>
        function openViewModal(id, name, email, phone, pets, accessories) {
        ...
        `).join('') :
                "<p>No pets supplied</p>";

            const accessoryList = document.getElementById('accessoryList');
            accessoryList.innerHTML = accessories.length ?
                accessories.map(a => {
                    let variantsHTML = '';
                    ...
                    if (a.variants && a.variants.length) {
                        variantsHTML = a.variants.map(v => {
                            ...
                            if (v.outlet_accessories) {
                                outlets = v.outlet_accessories.map(o =>
                                    `<span class="outlet-tag">${o.outlet.State} (${o.StockQty})</span>`
                                ).join('');
                            }

                            return `
                                <div class="bubble-container variant-row">
                                    <span class="bubble variant-tag">${v.VariantKey}</span>
                                    ${v.outlet_accessories ? v.outlet_accessories.map(o =>
                                        `<span class="bubble outlet-tag">${o.outlet.State} (${o.StockQty})</span>`
                                    ).join('') : ''}
                                </div>
                            `;
                        }).join('');
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\User\finalyear\resources\views/admin/supplier.blade.php ENDPATH**/ ?>